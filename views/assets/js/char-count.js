;(function(global) {
				...
				if (options && options.highlight) {
					...
					var countHighlightClass =
						countElement.type === 'text'
							? 'govuk-c-charcount__highlight-input'
							: 'govuk-c-charcount__highlight'
					wrapper.insertAdjacentHTML(
						'afterbegin',
						'<div id="' +
							elementId +
							'-hl" class="form-control ' +
							countHighlightClass +
	...
	CharCount.prototype.createCountMessage = function(countElement) {
		...
		if (elementId && !countMessage) {
			countElement.insertAdjacentHTML(
				...
				'<span id="' +
					elementId +
					'-info" class="form-hint govuk-c-charcount__message" aria-live="polite"></span>'
			)
	...
	CharCount.prototype.bindChangeEvents = function(countElementExtended) {
		if (countElementExtended.countElement.addEventListener) {
			...
			countElementExtended.countElement.attachEvent(
				'onkeyup',
				CharCount.prototype.handleInput.bind(countElementExtended)
			)
		}
		...
		countElementExtended.countElement.addEventListener(
			'focus',
			CharCount.prototype.handleFocus.bind(countElementExtended)
		)
		countElementExtended.countElement.addEventListener(
			'blur',
			CharCount.prototype.handleBlur.bind(countElementExtended)
		)
	}
