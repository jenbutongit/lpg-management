...
describe('Module Controller Tests', function() {
	...
	it('should remain on add module page if no course is selected', async function() {
		const setModule: (request: Request, response: Response, next: NextFunction) => void = moduleController.setModule()
		...
		course.id = 'abc123'
		response.locals.course = course
		await setModule(request, response, next)
	...
	it('should render add file page', async function() {
		const addFile: (request: Request, response: Response, next: NextFunction) => void = moduleController.addFile()

		const request: Request = mockReq()
		const response: Response = mockRes()

		await addFile(request, response, next)

		expect(response.render).to.have.been.calledOnceWith('page/course/module/module-file')
	})

	it('should delete module', async function() {
		const deleteModule: (request: Request, response: Response, next: NextFunction) => void = moduleController.deleteModule()
		...
		learningCatalogue.deleteModule = sinon.stub().returns(Promise.resolve())

		await deleteModule(request, response, next)
